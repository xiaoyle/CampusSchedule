...
@Composable fun StudyTaskDialog(task:StudyTask,occurrence:TaskOccurrence?,existing:Boolean,data:AppData,busy:Boolean,onDismiss:()->Unit,onSave:(StudyTask,Boolean)->Unit,onDelete:(Boolean)->Unit,onDuplicate:()->Unit,onStartFocus:()->Unit){
    ...
    },confirmButton={Button(onClick={submit()},enabled=!busy){if(busy)CircularProgressIndicator(Modifier.size(18.dp),strokeWidth=2.dp)else Text("保存")}},dismissButton={TextButton(onClick={requestDismiss()},enabled=!busy){Text("取消")}})
    ...
    if(discardConfirm)AlertDialog(onDismissRequest={discardConfirm=false},title={Text("待办尚未保存")},text={Text("你可以现在保存修改、放弃修改，或继续编辑。")},confirmButton={Button(onClick={submit()},enabled=!busy){Text("保存修改")}},dismissButton={Row{TextButton(onClick={discardConfirm=false;onDismiss()}){Text("放弃修改")};TextButton(onClick={discardConfirm=false}){Text("继续编辑")}}})
}
