...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-0.8.0-1976D2">
...
- 可新增、编辑、删除指定日期的一次课程，冲突时提示但不阻止保存；删除后可撤销。
...
- 个人中心按阶段展示 3 张中大校园实景照片，支持左右滑动；每日按北京时间更新一条 20 字以内的励志短句。
- 支持康乐园绿、珠海海蓝和四季自动配色；课程可选择自己的颜色。
- 普通图标启动时可长按芯片充能进入，并选择三套动态氛围场景或自己的照片；组件使用对应静态海报。
- 图片通过系统选择器导入并压缩到本机私有目录；最多收藏 8 套桌面搭配。

## 下载与使用

前往仓库右侧 **Releases**，打开 `v0.8.0`，在 **Assets** 中下载 `CampusSchedule-0.8.0.apk`。GitHub 自动生成的 Source code 不是安装包。
...
- 0.8.0 的锁屏计时、环境音、动画与多品牌真机效果由试用反馈继续验证。学校改版或限制 WebView 时可退回文件导入。

## 文档

- [0.8.0 发布说明](docs/RELEASE-v0.8.0.md)
