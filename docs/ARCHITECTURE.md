...
下一节课和今日课程两个组件分别保存 `DiyStyle`，内容包含样式、图片引用、裁剪位置、缩放、模糊、遮罩、透明度、标题、短句和字体比例。图片经系统选择器读取、校正方向、限制尺寸并重新编码到应用私有目录，不保存来源 URI 或 EXIF 信息。学习看板是第三个组件，读取全局主题、头像、课程统计和任务数据，不新增独立图片 DIY。
