...
`StudyTask` 保存任务类型、课程规则关联、课程名快照、截止时间、优先级、备注、提醒提前量和完成时间。编辑器可让用户选择任意提醒日期与时间，保存时换算为相对截止时间的分钟偏移；重复实例因此保持相同提醒规则。`StudyTaskEngine` 负责校验、排序、提醒时间计算和课程失联判断。任务与课表共享 `AppData` 的兼容序列化容器，但删除或重新导入课程不会级联删除任务。
...
0.11.0 增加独立的 `TaskTestReminder`，使用单独 PendingIntent 和临时状态验证课业待办通知渠道，不覆盖课程提醒、真实任务提醒或组件刷新；测试不会在重启后恢复。

`NoteCategory`、`StudyNote` 与 `NoteStyle` 作为带默认值字段保存在 `AppData` JSON 中，不改变 Room 表结构。栏目删除时笔记移动到未分类；笔记内容不参与导入、组件、提醒和排查信息。
