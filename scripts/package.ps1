...
Copy-Item 'app/build/outputs/apk/debug/app-debug.apk' (Join-Path $output 'CampusSchedule-0.3.0.apk') -Force
Copy-Item 'README.md' (Join-Path $output '安装与使用说明.md') -Force
Copy-Item 'VERIFICATION.md' (Join-Path $output '验证记录.md') -Force
Add-Type -AssemblyName System.IO.Compression
$archivePath = Join-Path $output 'CampusSchedule-source-0.3.0.zip'
...
Get-FileHash -Algorithm SHA256 (Join-Path $output 'CampusSchedule-0.3.0.apk'),$archivePath | ForEach-Object { "$($_.Hash)  $([System.IO.Path]::GetFileName($_.Path))" } | Set-Content -LiteralPath (Join-Path $output 'SHA256SUMS-0.3.0.txt') -Encoding utf8
